...
def test_levels_yields_to_breaks():
    ...
    with pytest.warns(FutureWarning, match="in favour of `breaks`"):
        data = p.build_test().built.layers[0].data
...
def test_levels_yields_to_bins():
    ...
    expected = (p0 + geom_density_2d(bins=5)).build_test().built.layers[0].data
...
def test_default_uses_more_than_three_contour_levels():
    data = (p0 + geom_density_2d()).build_test().built.layers[0].data
