...
- The default horizontal alignment for the plot title if there is no subtitle,
  is the center. If there is a subtitle, the default is to align both to the
  the left. If the horizontal alignment is set for only one of them, then it
  controls both. To control the two separately, set the alignments for each.
  e.g.

  ```py
  theme(
      plot_title=element_text(ha="left"),
      plot_subtitle=element_text(ha="right")
  )
  ```
...
- Improved support for customizing guides/legends.
  ...
  - You can now apply themes to individual guides, e.g.
    ...
    ```py
    + guides(guide_legend(theme=theme_minimal())
    ```
