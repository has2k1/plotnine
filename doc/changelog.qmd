...
- [](:class:`~plotnine.composition.plot_layout`) gained `axes` and `axis_title`
  parameters. Use them to show an axis, or an axis title, once for plots in a
  composition that share it.
...
- [](:class:`~plotnine.labs`) can now be used to create a `footer`.
  This footer can be styled using the `plot_footer` parameter to [](:class:`~plotnine.theme`).
...
- Added the `strip_placement` theme element which controls how a facet strip and
  a moved axis on the same side are arranged. The default `"inside"` keeps the
  strip against the panel and places the axis beyond it; `"outside"` places the
  strip beyond the axis.
...
- Added [](:class:`~plotnine.geom_contour`) and
  [](:class:`~plotnine.geom_contour_filled`) to represent a gridded surface
  with contour lines and filled bands.
  ({{< issue 1084 >}})

- Added [](:class:`~plotnine.geom_density_2d_filled`), the filled
  counterpart of [](:class:`~plotnine.geom_density_2d`).

- [](:class:`~plotnine.geom_polygon`) can now draw polygons with holes.
  Use the new `subgroup` aesthetic to identify each ring within a polygon.

- Plot components that take a `random_state` now also accept a
...
- Added [](:class:`~plotnine.geom_beeswarm`) and
  [](:class:`~plotnine.stat_beeswarm`), which arrange points along one variable
  and spread nearby values sideways into a swarm. Use the `spread` parameter to
  select `"quasirandom"` (the default), `"pseudorandom"`, `"smiley"`, or
  `"frowney"` placement.

- Added [](:meth:`~plotnine.ggplot.build`) to compute a plot's layers, scales
  and layout without drawing it.
...
- `contourpy>=1.2.0` is now required. It computes the contours of a surface.

- [](:class:`~plotnine.stat_density_2d`) gained `contour_var`, `bins`,
  `binwidth` and `breaks`. Every panel now uses the same contour heights.
  The stat divides density into ten bands by default instead of five, so
  [](:class:`~plotnine.geom_density_2d`) draws more contour lines for the
  same data. The `levels` parameter is deprecated; use `bins` or `breaks`
  instead.
...
- Specific theme elements now override blank general elements. For example,
  adding `theme(axis_text_x_bottom=element_text(size=14))` to
  [](:class:`~plotnine.theme_void`) displays the bottom x-axis tick labels and
  reserves space for them. Previously, the blank `axis_text_x` element
...
- Plots containing an unseeded [](:class:`~plotnine.geom_sina`) can now be
  copied, including when the geom has been added to every plot in a
  composition. Copying previously raised
  `TypeError: cannot pickle 'module' object`.
  ({{< issue 1115 >}})

- [](:class:`~plotnine.position_jitter`) and
  [](:class:`~plotnine.position_jitterdodge`) now make independent random
  draws for the horizontal and vertical axes when given a seed. Both axes
  previously received identical offsets, placing points along a diagonal.

- [](:class:`~plotnine.position_jitter`),
  [](:class:`~plotnine.position_jitterdodge`),
  [](:class:`~plotnine.stat_summary`), and
  [](:class:`~plotnine.stat_summary_bin`) now treat `0` as a valid
  `random_state` seed. They previously selected NumPy's global random state,
  so results depended on other random draws.

- Broadcasting a layer to every plot in a composition with `&` or `*` now
  gives each target plot an independent copy. Changes made while drawing one
  plot no longer affect the layer in another plot.

- Calling [](:meth:`~plotnine.ggplot.draw`) more than once on the same plot no
  longer duplicates artists or leaves ticks and labels partially hidden.
  ({{< issue 1009 >}})
...
-  Themeables
   ...
   `axis_ticks_pad_major_x` and
   `axis_ticks_pad_major_y` have been deprecated. Use the `margin` parameter
   of [](:class:`~plotnine.element_text`) with `axis_text`, `axis_text_x`
   or `axis_text_y` to control the spacing between the axis text and the ticks.
   ({{< issue 843 >}})
...
- Plot Composition is here. Using an intuitive algebra, you can
  now put combine multiple plots into a single graphic.  e.g.
...
- [](:class:`~plotnine.themes.themeable.legend_key`) now inherits from 
  [](:class:`~plotnine.themes.themeable.panel_background`) and the default themes have been
  adjusted to rely on this inheritance. So when using the default themes, altering `fill`
  parameter of the `panel_background` will also affect the background of the `legend_key`.

- The default value for the `guide` parameter for the identity scales (`scale_*_identity`)
  has been reverted to `None`. This means by default using these scales will not generate
  a legend.
...
- Fixed bug where justifying `axis_text_x` and `axis_text_y` using horizontal and vertical
  alignment did not work as expected. ({{< issue 927 >}})

- Fixed justifying strip_text within the strip_background. ({{< issue 867 >}})
...
- Fixed bug in all manual scales (`scale_*_manual`) where if you set `values`
  to a `dict` then setting the breaks would lead to an error. ({{< issue 949 >}})

- Fixed using [](:class:`~plotnine.theme_void`) with facetting so that the
  strip text are always fully within the figure bounds.

- Fixed the ordinal color scales [](:class:`~scale_color_ordinal`), [](:class:`~scale_color_cmap_d`)
  [](:class:`~scale_fill_ordinal`) and [](:class:`~scale_fill_cmap_d`)
  to handle missing (`NaN` or `None`) values. ({{< issue 953 >}})
