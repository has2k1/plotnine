...
def test_geom_basics():
    ...
    with assert_raises(GgplotError):
        g = geom(aes(color="beef"), aes(color="pork"))
    assert_equal("pork", g.mapping["color"])
    # setting, not mapping
    g = geom(color="blue")
    assert_equal("blue", g.aes_params["color"])
...
#     df.is_copy = None
#     g2 = gg + geom_text(aes(label="name"), data=df)
#     print(g2)
...
def test_geom_with_data():
    ...
    g2 = gg + _text
    assert_is_not(g2.data, _text.data,
                  "Adding a dataset to a geom replaced the data in ggplot")
