...
df = pd.DataFrame(
    {'slope': [1, 1], 'intercept': [1, -1], 'x': [-1, 1], 'y': [-1, 1], 'z': range(2)}
)
...
def test_aesthetics():
    p = (
        ...
        + geom_abline(
            aes(slope='slope', intercept='intercept+.2', linetype='factor(z)'), size=2
        )
        + geom_abline(
            aes(slope='slope', intercept='intercept+.3', color='factor(z)'), size=2
        )
...
def test_aes_inheritance():
    ...
    p = (
        ggplot(
            df, aes('x', 'y', color='factor(z)', slope='slope', intercept='intercept')
        )
