...
def test_mean_cl_normal():
    p = ggplot(df, aes('x', 'y')) + stat_summary(fun_data='mean_cl_normal', size=2)

    assert p == 'mean_cl_normal'


def test_mean_sdl():
    p = ggplot(df, aes('x', 'y')) + stat_summary(fun_data='mean_sdl', size=2)

    assert p == 'mean_sdl'


def test_median_hilow():
    p = ggplot(df, aes('x', 'y')) + stat_summary(fun_data='median_hilow', size=2)

    assert p == 'median_hilow'


def test_mean_se():
    p = ggplot(df, aes('x', 'y')) + stat_summary(fun_data='mean_se', size=2)
