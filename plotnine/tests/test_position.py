...
def test_jitter():
    ...
    p = (
        ...
        + geom_jitter(
            size=10, color='blue', width=0.1, height=0.1, random_state=random_state
        )
    )
...
def test_stack_negative():
    ...
    p = (
        ...
        + geom_text(
            aes('factor(x)', 'y', label='y'), position=position_stack(vjust=0.5)
        )
    )
...
def test_fill():
    p = ggplot(df2, aes('factor(z)')) + geom_bar(aes(fill='factor(x)'), position='fill')
...
def test_dodge2():
    p = ggplot(df3, aes('x', 'y', color='c')) + geom_boxplot(position='dodge2', size=2)
