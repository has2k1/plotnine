...
from plotnine.scales.scale_alpha import scale_alpha_discrete, scale_alpha_continuous
from plotnine.scales.scale_linetype import (
    scale_linetype_discrete,
    scale_linetype_continuous,
)
from plotnine.scales.scale_shape import scale_shape_discrete, scale_shape_continuous
...
def test_expand_limits():
    ...
    p = ggplot(aes('x', 'y'), data=df) + geom_point() + expand_limits(y=(0, None))
...
def test_scale_without_a_mapping():
    ...
    p = ggplot(df, aes('x', 'x')) + geom_point() + scale_color.scale_color_continuous()
    with pytest.warns(PlotnineWarning):
        p.draw_test()
...
def test_missing_manual_dict_aesthetic():
    df = pd.DataFrame({'x': range(15), 'y': range(15), 'c': np.repeat(list('ABC'), 5)})
...
def test_missing_data_discrete_scale():
    ...
    p = ggplot(df, aes('a', 'a')) + geom_point(aes(fill='b'), stroke=0, size=10)
    assert p + _theme == 'missing_data_discrete_scale'


df = pd.DataFrame({'x': range(4), 'y': range(4), 'w': list('wxyz'), 'z': list('abcd')})
...
def test_legend_ordering_local_aethetics_1():
    # 1. color
    # 2. shape
    p = ggplot(df) + aes('x', 'y') + geom_point(aes(color='w', shape='z'), size=5)
...
def test_legend_ordering_local_aethetics_2():
    # 1. shape
    # 2. color
    p = ggplot(df) + aes('x', 'y') + geom_point(aes(shape='z', color='w'), size=5)
...
def test_legend_ordering_mixed_scope_aesthetics():
    ...
    # 1. color
    # 2. shape
    p = ggplot(df) + aes('x', 'y', shape='z') + geom_point(aes(color='w'), size=5)
...
def test_datetime_scale_limits():
    ...
    df = pd.DataFrame(
        {'x': [datetime(x, 1, 1) for x in range(2000, 2000 + n)], 'y': range(n)}
    )
...
def test_ordinal_scale():
    ...
    p = ggplot(df) + aes('x', 'y', color='-y', fill='x') + geom_col(size=4) + _theme
